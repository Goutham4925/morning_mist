...
export default function Architecture() {
  ...
  const activeCaption = captions.filter((c) => p >= c.at).pop() ?? captions[0];
  ...
  return (
    <section ref={ref} id="architecture" className="relative h-[260vh] bg-fog">
      <div className="sticky top-0 flex h-screen flex-col overflow-hidden">
        <div className="relative mx-auto grid w-full max-w-7xl flex-1 items-center gap-10 px-6 pt-28 lg:grid-cols-[2fr_3fr] lg:px-10">
          {/* narrative */}
          <div>
            <p className="timestamp text-lg text-bronze">09:45 — the draughtsman&apos;s hour</p>
            <h2
              key={activeCaption.title}
              className="font-display mt-5 text-4xl font-light leading-[1.08] tracking-tight text-ink transition-opacity duration-500 sm:text-6xl"
            >
              {activeCaption.title.split(" ").slice(0, -1).join(" ")}{" "}
              <span className="italic text-moss">
                {activeCaption.title.split(" ").slice(-1)}
              </span>
            </h2>
            <p
              key={activeCaption.text}
              className="mt-8 max-w-md text-base font-normal leading-relaxed text-eucalyptus transition-opacity duration-500"
            >
              {activeCaption.text}
            </p>

            <div className="mt-10 hidden h-px w-48 overflow-hidden bg-ink/10 lg:block">
              <div
                className="h-full bg-bronze transition-[width] duration-150"
                style={{ width: `${p * 100}%` }}
              />
            </div>
            <p className="mt-3 hidden text-[10px] uppercase tracking-[0.3em] text-eucalyptus lg:block">
          ...
          >
            ...
            >
              ...
              <path d="M110 450 V190 H250 V450" pathLength={1} style={stroke(0.08, 0.3)} />
              <path d="M380 450 V150 H520 V450" pathLength={1} style={stroke(0.14, 0.36)} />
              <path d="M650 450 V190 H790 V450" pathLength={1} style={stroke(0.2, 0.42)} />
